...
####################################################################################################
create table countries (name char (12),population int,capital char(12));
...
insert into countries (name,population,capital)
values('india',1326,'delhi');
insert into countries (name,population,capital)
values('unitedstates',324,'washingtondc');
insert into countries (name,population,capital)
values('Indonesia',260,'Jakarta');
insert into countries (name,population,capital)
values('Brazil',209,'Brasilia');
insert into countries (name,population,capital)
values('Pakistan',193,'Islamabad');
insert into countries (name,population,capital)
values('Nigeria',187,'Abuja');

####################################################################################################
# chane the name of delhi to new delhi
...
#supplier
create table suppliers (supp_id int primary key,supp_name varchar(30));
 #product table
 create table product 
 (pro_ID int primary key,pro_name varchar(30) not null unique,supp_id int references suppiers(id));
  #create table of stock
  create table stock(stock_id int primary key,pro_id int references product(id));
  ###################################################################################################################
  
#6Enter some records into the three tables.

  insert into suppliers values(101,"allen");
  insert into suppliers values(102,"suraj");
  insert into product values(11,"lamp",101);
  insert into product values(12,"mirror",102);
  insert into stock values(1001,11);
  insert into stock values(1002,12);
  
#7. Modify the supplier table to make supplier name unique and not null.
alter table suppliers modify supp_name varchar(30) not null unique;
#######################################################################
...
alter table emp add emp_id int;
####################################################################################################
#Set the value of deptno in the following order
create table employee (emp_id integer, fname varchar(30), lname varchar(30), deptno tinyint, salary decimal(10,2));
...
#Create a view called emp_sal on the emp table by selecting the following fields in the order of highest salary to the lowest salary.
 # salary asending to decending
 select*from employee;
SELECT * FROM Employee ORDER BY salary DESC;

SELECT * FROM Employee ORDER BY emp_id,fname,lname,salary DESC;
